...
async function handleRequest(request) {
  ...
  try {
    ...
    if (!calendarResponse.ok) {
      ...
      console.error(
        `Error fetching events: Status ${calendarResponse.status}, Message: ${errorText}`,
      );
      return new Response(
        `Failed to fetch events: ${calendarResponse.status}`,
        {
    ...
    if (!sheetsResponse.ok) {
      ...
      console.error(
        `Error fetching sheet data: Status ${sheetsResponse.status}, Message: ${errorText}`,
      );
      return new Response(
        `Failed to fetch sheet data: ${sheetsResponse.status}`,
        {
